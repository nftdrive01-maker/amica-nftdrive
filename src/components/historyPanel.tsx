...
export function HistoryPanel({
  ...
  const [domainOptionMap, setDomainOptionMap] = useState<Record<string, PublicDomainOption>>({});
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([
      chatHistoryStore.listDomainIds(),
      fetchPublicDomainOptions(),
    ]).then(([items, domainOptions]) => {
      if (!active) {
        return;
      }
      setDomainIds(items);
      setDomainOptionMap(
        Object.fromEntries(domainOptions.map((domain) => [domain.id, domain])),
      );
    }).catch((error: unknown) => {
      if (!active) {
        return;
  ...
  const resolveDomainLabel = (domainId: string) => {
    const domain = domainOptionMap[domainId];
    return domain?.characterName?.trim() || domain?.label?.trim() || domainId;
  };
  ...
  return (
    <>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-28 top-14 z-40 flex justify-end px-2 sm:inset-y-0 sm:right-0 sm:px-0 sm:items-stretch">
        <div className="pointer-events-auto flex h-full w-full max-w-[min(96vw,700px)] flex-col overflow-hidden rounded-lg border border-slate-700/60 bg-slate-900/80 shadow-lg backdrop-blur-md transition-all duration-300 sm:w-[min(94vw,700px)] lg:mb-12 lg:h-[88%] lg:w-[34vw] xl:w-[32vw] translate-x-0 opacity-100">
                  ...
                  <label className="grid gap-1 text-sm text-white/86">
                    ...
                    >
                      ...
                      {domainIds.map((domainId) => (
                        <option key={domainId} value={domainId}>{resolveDomainLabel(domainId)}</option>
                      ))}
                    </select>
                  </label>
